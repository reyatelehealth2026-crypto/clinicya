...
class OdooWebhookHandler {
    ...
    private function awardInvoicePoints($invoiceData) {
        ...
        // ── แต้ม: ผ่าน ledger เท่านั้น ────────────────────────────────────────
        // BATCH 2. This used to increment users.points + available_points +
        // total_points itself and hand-roll balance_after from users.points — the
        // number the customer was then TOLD in the LINE message below, even though
        // it already included welcome bonuses and shop awards that never reached
        // available_points. The check-then-act dedupe above is also unindexed, so
        // two concurrent deliveries could both pass it; the idempotency key is
        // backed by a UNIQUE index and cannot.
