...
function ensureCartProductSourceSupport(PDO $db): void
{
    ...
    try {
        if (!hasTableColumn('cart_items', 'product_source')) {
            $db->exec("ALTER TABLE cart_items ADD COLUMN product_source VARCHAR(32) NOT NULL DEFAULT 'business_items' COMMENT 'business_items|shop_products' AFTER product_id");
        }
    } catch (Exception $e) {
...
function loadCheckoutCartLinesFromDb(PDO $db, int $userId, ?int $lineAccountId): array
{
    ...
    if (hasTableColumn('cart_items', 'product_source') && tableExists('shop_products')) {
        $stmt = $db->prepare(
            'SELECT c.*,
                    p.name AS bi_name,
                    p.price AS bi_price,
...
function handleAddToCart($data) {
    ...
    if ($productSource === 'shop_products' && !hasTableColumn('cart_items', 'product_source')) {
        jsonResponse(false, 'Cart migration required (product_source)');
    }

    if ($productSource === 'shop_products') {
        if (!tableExists('shop_products')) {
            jsonResponse(false, 'Product not found');
        }
        $stmt = $db->prepare(
...
function handleUpdateCart($data) {
    ...
    if ($quantity <= 0) {
        ...
        if ($productSource === 'shop_products' && tableExists('shop_products')) {
            $stmt = $db->prepare(
                'SELECT saleable_qty FROM shop_products WHERE id = ? AND line_account_id = ? AND storefront_enabled = 1 AND is_active = 1'
            );
            $stmt->execute([$productId, $lineAccountId]);
...
function handleCreateOrder($data) {
    ...
    try {
        ...
        foreach ($items as $item) {
            ...
            if ($src === 'shop_products' && tableExists('shop_products')) {
                $stmt = $db->prepare(
                    'UPDATE shop_products SET saleable_qty = saleable_qty - ?, updated_at = NOW()
...
function handleUploadSlip() {
    ...
    // Get order info — scoped to the current tenant DB only.
    // SECURITY: the cross-tenant fallback scan that used to live here was an
    // IDOR (same class as handleGetOrder) — it let a caller upload a slip onto
    // any tenant's order by guessing the sequential id. Removed. If the order
    // isn't in this tenant's DB, it doesn't belong to this caller.
    ...
    // Build the slip URL from the host that actually received this upload — that
    // is where the file was just written and is served from. The hardcoded
    // BASE_URL points at a single tenant subdomain whose docroot does NOT contain
    // this shared upload dir, so using it makes the image 404 on BOTH the app and
    // the admin order page. The request host is always correct and loads fine
    // cross-subdomain (absolute URL).
    $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $host = $_SERVER['HTTP_HOST'] ?? (defined('BASE_URL') ? parse_url(BASE_URL, PHP_URL_HOST) : '');
    $baseUrl = $host ? ($scheme . '://' . $host) : rtrim(defined('BASE_URL') ? BASE_URL : '', '/');
...
function handleGetOrder() {
    ...
    // The order lookup is scoped to the current tenant DB (resolved by
    // bootstrap/route_by_account.php). We intentionally do NOT scan other
    // tenants' databases here — doing so was a cross-tenant IDOR that let any
    // caller enumerate orders (customer name, address, items) across every
    // tenant by guessing the sequential `id`. If the order is not in this
    // tenant's DB, it does not belong to this caller.
    if (!$order) {
        jsonResponse(false, 'Order not found');
    }
    ...
    // Latest payment slip for this order — lets the app show the uploaded slip
    // image and an "awaiting verification" state instead of asking the customer
    // to upload again. slip_status: pending | approved | rejected.
    $order['slip_image'] = null;
    $order['slip_status'] = null;
    try {
        $ss = $db->prepare("SELECT image_url, status FROM payment_slips WHERE transaction_id = ? ORDER BY created_at DESC LIMIT 1");
        $ss->execute([$orderId]);
        $slipRow = $ss->fetch(PDO::FETCH_ASSOC);
        if ($slipRow) {
            $order['slip_image'] = reyaFixSlipUrl($slipRow['image_url'] ?: null);
            $order['slip_status'] = $slipRow['status'] ?: null;
        }
    } catch (Throwable $e) {
        // payment_slips may not exist on the oldest schema — non-fatal.
    }
