...
### 4. โบนัสต้อนรับที่เคยแจกไปแล้ว — ตัดทิ้ง ไม่ต้องโอน

**เจ้าของตัดสิน 2026-09-07: 50 แต้มที่เคยแจกไป ตัดทิ้งได้เลย**

ยกเลิกโบนัสแบบย้อนหลัง ไม่ใช่แค่หยุดแจกไปข้างหน้า `users.points` จึงถูก
ศูนย์ทิ้งได้ตรง ๆ ไม่ต้องโอนเข้า `points_transactions` ก่อน

ผลกระทบจริงจากตัวเลขที่สำรวจ (ไม่นับ tenant 0001):

- **23 คนใน 0003 ไม่รู้สึกอะไรเลย** — ทั้งคู่มีทั้ง `points = 50` และ
  `available_points > 0` อยู่แล้ว พอสลับ reader ไปอ่านยอดจริง เลขที่เห็นจะ
  **เพิ่มขึ้น** (เช่น 50 → 320) เพราะเดิมการ์ด LINE โชว์แค่ 50
- **5 คนเห็นแต้มหาย 50 → 0** — 1 คนใน 0003, 2 คนใน 0013, คนละ 1 ใน 0007 และ
  0101 ทั้งหมดคือคนที่ไม่เคยสะสมแต้มจริงเลยนอกจากโบนัส

รวมทั้งแพลตฟอร์ม 1,400 แต้ม ใน 28 คน

### 5. คอลัมน์และตารางที่เลิกใช้
...
เพราะนั่นคือการแจกโบนัสที่กำลังยกเลิกซ้ำอีกรอบ และเจ้าของตัดสินแล้วว่า
**50 ที่เคยแจกไปตัดทิ้งได้** (ดูหัวข้อ Decision ข้อ 4) เฟสที่เคยเป็น
"จุดที่ย้อนยาก" จึงหายไปจากแผนทั้งเฟส
...
**deploy โค้ดขึ้น prod แล้ว 2026-09-07** — 20 ไฟล์ PHP ของเฟส 1/3/4 สำรองไว้ที่
`/home/zrismpsz/_deploy_backups/pre-adr008-20260907-032003.tgz` ก่อนทับ
เทียบทุกไฟล์กับ `origin/main` ก่อนส่ง: 18 ไฟล์เหมือนกันเป๊ะ อีก 2 ไฟล์
(`webhook.php`, `shop/order-detail.php`) prod **ตามหลัง** main ไม่ใช่ล้ำหน้า
จึงไม่มีโค้ดเฉพาะ prod สูญ

smoke test บน tenant 0003 หลัง deploy: คลาสอ่านยอดตรงกับ ledger ทุกคนที่สุ่มมา
และผู้ใช้ที่เคยถือแต่โบนัสอย่างเดียวอ่านได้ 0 แล้ว — fallback ที่เคยชุบชีวิต 50
กลับมาหายไปจริงบนเครื่องจริง

`LoyaltyPoints` เช็ค `SHOW COLUMNS` ก่อนใช้คีย์ ทั้งสองทิศจึงปลอดภัยระหว่าง
deploy: โค้ดใหม่บน DB เก่าก็ได้ DB ใหม่กับโค้ดเก่าก็ได้
...
ทำได้เลยโดยไม่ต้องเตรียมอะไรก่อน

ถึงอย่างนั้นยัง **snapshot ตาราง `users` ต่อ tenant ก่อนรันเฟส 5** — DROP คอลัมน์
ย้อนไม่ได้ และ tenant 0001 ยังมียอด legacy ที่ยังไม่ได้ตัดสิน (ดู Known Gaps)
...
- ไม่มีแต้มที่ระบบบอกว่าให้แล้วแต่ใช้ไม่ได้อีก — เดิมโบนัสต้อนรับตอบกลับว่า
  ได้ 50 แต้ม โดยลงคอลัมน์ที่มินิแอปไม่อ่าน
...
- **ลูกค้า 5 คนจะเห็นแต้มหาย 50 → 0** ตอนสลับ reader ในเฟส 3 (1 คนใน 0003,
  2 คนใน 0013, คนละ 1 ใน 0007 กับ 0101) เป็นผลโดยตรงจาก Decision ข้อ 4
  ควรบอกร้านล่วงหน้า ไม่ใช่ให้ไปเจอเองตอนลูกค้าทัก
- ในทางกลับกัน อีก 23 คนใน 0003 จะเห็นแต้ม **เพิ่มขึ้น** (เช่น 50 → 320)
  เพราะการ์ด LINE เดิมโชว์แค่โบนัส ไม่ได้โชว์ยอดจริง — เตรียมคำอธิบายด้วย
- ระหว่างที่ฝั่ง Next ยังไม่ทำ จะมีช่วงที่โค้ดสองแบบอยู่ด้วยกัน และ
  `/api/miniapp` ที่ live คือฝั่งเก่า (ดู Known Gaps)
