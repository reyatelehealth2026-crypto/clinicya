...
/**
 * session.ts — auth/tenant-db resolution for the /api/inbox/actions/get-admins
 * Route Handler (port of api/inbox-v2.php's `case 'get_admins':`, lines
 * ~2434-2456).
 *
 * Same "who is logged in + which tenant DB" gate as
 * api/inbox/conversations/_lib/session.ts and every sibling
 * actions/{tags,notes,medical,send-message}/_lib/session.ts copy (all
 * mirror inbox-v2.php's `require_once 'includes/header.php'` gate at line
 * 991 — any authenticated admin, no page-specific role restriction).
 * Returns a JSON-shaped auth result rather than redirect()ing — this is a
 * same-page AJAX action endpoint (inbox-v2.php's action switch), consumed
 * by client-side `fetch()`, not a page render, so an unauthenticated/expired
 ...
 * medical/send-message copies — same "every consumer resolves its own
 * session" rationale documented on users/_lib/session.ts, and this batch's
 * ownership boundary keeps api/inbox/actions/get-admins/** independently
 * editable from sibling action families (assign-conversation,
 * unassign-conversation, get-assignment included) without cross-imports.
 */
