...
/**
 * session.ts — auth/tenant-db resolution for the /api/inbox/actions/poll
 * Route Handler (port of api/inbox-v2.php's `case 'poll':`, lines ~172-193,
 * backed by `InboxService::pollUpdates()`).
 *
 ...
 * client-side `fetch()`/polling, not a page render, so an unauthenticated/
 * expired session request must get back a JSON 401 the caller can react to,
 * not an HTML redirect response.
 *
 ...
 * api/inbox/actions/{poll,save-pending-order,update-chat-status}/**
