...
/**
 * session.ts — auth/tenant-db resolution for the /api/inbox/actions/customer-crm
 * Route Handler (port of api/inbox-v2.php's `case 'customer_crm':`, lines
 * ~1905-1961, the CRM HUD panel's aggregate customer-data endpoint).
 *
 ...
 * api/inbox/actions/{customer-crm,add-customer-note,add-customer-tag,
 * remove-customer-tag,update-customer-info,customer-loyalty}/** independently
 * editable from sibling action families (in particular `assign-tag/**` and
 * `notes/**`, the two already-merged action families this batch's brief
 * explicitly warns not to touch) without cross-imports.
 */
