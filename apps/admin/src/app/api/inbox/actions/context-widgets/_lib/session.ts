...
/**
 * session.ts — auth/tenant-db resolution for the /api/inbox/actions/context-widgets
 * Route Handler (port of api/inbox-v2.php's `case 'context_widgets': case
 * 'context-widgets': case 'get_context_widgets':`, lines ~1521-1564, backed
 * by `ConsultationAnalyzerService::getContextWidgets()`).
 *
 ...
 * being the ONE cross-import WITHIN this batch: `../quick-actions/_lib/
 * quickActions.ts` importing `detectStage` from
 * `../consultation-stage/_lib/consultationStage` — this route is not part
 * of that exception).
 */
