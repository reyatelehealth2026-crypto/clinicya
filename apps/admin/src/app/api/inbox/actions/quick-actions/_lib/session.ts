...
/**
 * session.ts — auth/tenant-db resolution for the /api/inbox/actions/quick-actions
 * Route Handler (port of api/inbox-v2.php's `case 'quick_actions': case
 * 'quick-actions': case 'get_quick_actions':`, lines ~1604-1642, backed by
 * `ConsultationAnalyzerService::getQuickActions()` — and, when the `stage`
 * query param is empty, transitively by `::detectStage()` too).
 *
 ...
 * (inbox-v2.php's action switch), consumed by client-side `fetch()`, not a
 ...
 * api/inbox/actions/{context-widgets,consultation-stage,quick-actions,
 * detect-urgency}/** independently editable from sibling action families
 * without cross-imports (the deliberate exception, per this batch's brief,
 * being the ONE cross-import WITHIN this batch: this route's own
 * `_lib/quickActions.ts` importing `detectStage` from
 * `../consultation-stage/_lib/consultationStage`).
 */
