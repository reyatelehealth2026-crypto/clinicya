 ...
 * /api/inbox/actions/unassign-conversation Route Handler (port of
 * api/inbox-v2.php's `case 'unassign_conversation':`, lines ~2529-2559).
 *
 * Same "who is logged in + which tenant DB" gate as
 * api/inbox/conversations/_lib/session.ts and every sibling
 * actions/{tags,notes,medical,send-message,get-admins,assign-conversation}/_lib/session.ts
 * copy (all mirror inbox-v2.php's `require_once 'includes/header.php'` gate
 * at line 991 — any authenticated admin, no page-specific role
 * restriction). Returns a JSON-shaped auth result rather than
 * redirect()ing — this is a same-page AJAX action endpoint (inbox-v2.php's
 * action switch), consumed by client-side `fetch()`, not a page render, so
 * an unauthenticated/expired session request must get back a JSON 401 the
 * caller can react to, not an HTML redirect response.
 *
 * Duplicated (not imported) from the conversations/messages/tags/notes/
 * medical/send-message/get-admins/assign-conversation copies — same "every
 * consumer resolves its own session" rationale documented on
 * users/_lib/session.ts, and this batch's ownership boundary keeps
 * api/inbox/actions/unassign-conversation/** independently editable from
 * sibling action families (get-admins, assign-conversation, get-assignment
 * included) without cross-imports.
 */
