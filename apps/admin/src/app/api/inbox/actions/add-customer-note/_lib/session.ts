...
/**
 * session.ts — auth/tenant-db resolution for the /api/inbox/actions/add-customer-note
 * Route Handler (port of api/inbox-v2.php's `case 'add_customer_note':`,
 * lines ~2026-2049).
 *
 ...
 * ROOT `inbox-v2.php`'s same-page-AJAX `save_note` case, a different PHP
 * file's switch entirely) — see `_lib/addCustomerNote.ts`'s module doc for
 * the full distinction. This file is not imported by, and does not import
 * from, `actions/notes/_lib/session.ts`.
 */
