...
/**
 * session.ts — auth/tenant-db resolution for the POST /api/inbox/actions/upload-for-analysis
 * Route Handler. Ports the auth gate that guards inbox-v2.php's `case 'upload_for_analysis':`
 * (lines 836-874), the same-page AJAX action gated on `$_SERVER['HTTP_X_REQUESTED_WITH']` in the
 * original (inbox-v2.php line ~991) — that gate itself runs only after `includes/header.php` has
 * already required a logged-in admin session for the page. This Route Handler is reachable
 * directly (not wrapped by `(tenant)/layout.tsx`, which only gates page renders), so it must
 * perform that check itself.
 *
 * NOTE: `case 'upload_for_analysis':` itself never touches `$db` (no query, no LINE call — it
 * only validates + saves the uploaded file to disk) — but the Route Handler still resolves this
 * full context (including `db`/`tenantId`) because the auth boundary is what matters here, not
 * whether the downstream action happens to need a DB handle. See route.ts's own doc comment.
 *
 * Requires any of the six TenantRole values — `case 'upload_for_analysis':` has no page-specific
 * role restriction beyond "any authenticated admin" (same as send_message/dispense/send_image).
 *
 * Duplicated (not imported) from send-message/_lib/session.ts, send-image/_lib/session.ts, etc. —
