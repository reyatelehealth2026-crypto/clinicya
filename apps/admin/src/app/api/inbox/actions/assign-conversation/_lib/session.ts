 ...
 * /api/inbox/actions/assign-conversation Route Handler (port of
 * api/inbox-v2.php's `case 'assign_conversation':`, lines ~2461-2519).
 *
 * Same "who is logged in + which tenant DB" gate as
 * api/inbox/conversations/_lib/session.ts and every sibling
 * actions/{tags,notes,medical,send-message,get-admins}/_lib/session.ts copy
 * (all mirror inbox-v2.php's `require_once 'includes/header.php'` gate at
 ...
 * medical/send-message/get-admins copies — same "every consumer resolves
 * its own session" rationale documented on users/_lib/session.ts, and this
 * batch's ownership boundary keeps
 * api/inbox/actions/assign-conversation/** independently editable from
 * sibling action families (get-admins, unassign-conversation,
 * get-assignment included) without cross-imports.
 */
