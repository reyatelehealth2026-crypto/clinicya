...
import { sendImageAction } from './_lib/sendImage';

/**
 * POST /api/inbox/actions/send-image — port of inbox-v2.php's `case 'send_image':` (lines
 * 736-834), the chat composer's "send image" upload button. Reads `multipart/form-data`
 * (`user_id` + an `image` file field) via `request.formData()` — this is a new endpoint, not
 * bound to PHP's `$_FILES`/`$_POST` superglobal shape.
 *
 ...
 * validation branch inside `sendImageAction()` returns that literal shape directly; the try/catch
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const result = await sendImageAction(db, session, form, origin);
    return NextResponse.json(result.body, { status: result.status });
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    return NextResponse.json({ success: false, error: `Failed to send image: ${errorMessage}` }, { status: 400 });
  }
}
