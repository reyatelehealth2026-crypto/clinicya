 ...
 * POST /api/inbox/actions/assign-tag Route Handler. Ports
 * api/inbox-v2.php's `case 'assign_tag':` (lines 2130-2153) — a distinct
 * JSON-API action from the same-page-AJAX `update_tags` action already
 * ported at `actions/tags` (a different PHP source file entirely; see
 * `_lib/assignTag.ts`'s module doc for the full distinction). This
 * directory does not import from, or otherwise reference, that other
 * action family's files.
 *
 * Same "who is logged in + which tenant DB" gate as the sibling
 * api/inbox/actions/{medical,notes,send-message,mark-all-read,mark-as-read-on-line}/_lib/session.ts
 ...
 * Requires any of the six TenantRole values — assign_tag has no
 ...
 * send-message/mark-all-read/mark-as-read-on-line copies — same "every
 * consumer resolves its own session" rationale documented on
 * users/_lib/session.ts, and this batch's ownership boundary keeps
 * api/inbox/actions/mark-all-read/**, .../mark-as-read-on-line/**, and
 * .../assign-tag/** independently editable without cross-imports (and
 * without reaching into any other action family's directory).
 */
