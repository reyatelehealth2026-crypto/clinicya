...
/**
 * session.ts — auth/tenant-db resolution for the /api/inbox/actions/analyze-symptom
 * Route Handler (port of api/inbox-v2.php's `case 'analyze_symptom': case
 * 'analyze-symptom':`, lines ~199-235, backed by
 * `PharmacyImageAnalyzerService::analyzeSymptom()`).
 *
 ...
 * api/inbox/actions/{analyze-symptom,analyze-drug,analyze-prescription}/**
 * independently editable from sibling action families without cross-imports
 * (the one deliberate exception being analyze-drug/analyze-prescription
 * importing `identifyDrug`/`ocrPrescription` directly from
 * `../../analyze-symptom/_lib/imageAnalyzer` — see that module's doc and
 * each route.ts's own doc, mirroring the precedent set by Phase 4 batch 4a's
 * drug-info -> max-discount import).
 */
