...
/**
 * session.ts — auth/tenant-db resolution for the /api/inbox/actions/create-template
 * Route Handler (port of api/inbox-v2.php's `case 'create_template':`,
 * lines 2237-2270).
 *
 * Same "who is logged in + which tenant DB" gate as
 * api/inbox/actions/get-admins/_lib/session.ts and every sibling
 * actions/{add-customer-note,drug-info,customer-crm}/_lib/session.ts copy
 * (all mirror inbox-v2.php's `require_once 'includes/header.php'` gate —
