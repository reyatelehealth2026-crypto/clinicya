 ...
 * POST /api/inbox/actions/send-batch-messages Route Handler (port of
 ...
 * `send_batch_messages` has no page-specific role restriction in the PHP
 * source either.
 *
 * SAFETY NOTE: this action pushes real LINE messages via `pushMessage()`
 * (never a reply token) — see `_lib/sendBatchMessages.ts`'s module doc for
 * the full validation/build/send pipeline. This file only resolves who is
 * allowed to reach the route at all.
 *
 ...
 * documented on `poll/_lib/session.ts` and `send-message/_lib/session.ts`,
