 ...
 * connection.release()`.
 *
 * Local copy of `send-message/_lib/testHelpers/fakeTenantDb.ts` (itself a
 * local copy of `api/inbox/conversations/_lib/testHelpers/fakeTenantDb.ts`)
 ...
 * own copy too, and this batch's ownership boundary keeps this action
 * folder independently editable). Test-only code, not worth a cross-feature
 * import.
 */
 ...
 * — default `() => []` (empty result set), override per test as needed. For
 * INSERT statements, mysql2's `OkPacket` shape (`{insertId, affectedRows}`)
 * must be returned instead so Kysely's mysql driver recognizes it as a
 * write result (see `isOkPacket()` in kysely's mysql-driver — it requires
 * BOTH `insertId` and `affectedRows` keys to be present).
