...
/**
 * session.ts — auth/tenant-db resolution for the /api/inbox/actions/check-drug-interactions
 * Route Handler (port of api/inbox-v2.php's
 * `case 'check_drug_interactions': case 'check-drug-interactions':`, lines ~1363-1401, backed by
 * `DrugRecommendEngineService::checkInteractions()` (its own, independent
 * interaction algorithm — see `_lib/checkDrugInteractions.ts`'s module doc).
 *
