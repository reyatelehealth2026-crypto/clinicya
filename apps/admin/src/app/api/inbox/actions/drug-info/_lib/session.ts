...
/**
 * session.ts — auth/tenant-db resolution for the /api/inbox/actions/drug-info
 * Route Handler (port of api/inbox-v2.php's `case 'drug_info': case
 * 'drug-info': case 'get_drug_info':`, lines ~518-612).
 *
 ...
 * api/inbox/actions/{drug-info,search-drugs,drug-pricing,max-discount,
 * drug-pricing-data,low-stock-drugs,drug-inventory}/** independently editable
 * from sibling action families without cross-imports (the one deliberate
 * exception being max-discount/_lib/drugPricingEngine.ts, which drug-info
 * imports directly — see that module's doc and drug-info/route.ts's doc).
 */
