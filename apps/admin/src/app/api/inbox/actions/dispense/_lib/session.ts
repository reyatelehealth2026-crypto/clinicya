...
/**
 * session.ts — auth/tenant-db resolution for the POST /api/inbox/actions/dispense Route
 * Handler. Ports the auth gate that guards inbox-v2.php's `case 'dispense':` (lines 469-736),
 ...
 * itself.
 *
 * Requires any of the six TenantRole values — `case 'dispense':` has no page-specific role
 * restriction beyond "any authenticated admin" (same as send_message/save_medical/save_note).
 *
 * Duplicated (not imported) from send-message/_lib/session.ts, notes/_lib/session.ts, etc. —
 * established repo precedent ("every consumer resolves its own session", see those files' own
 * doc comments) and this batch's ownership boundary (dispenseChain owns
 * apps/admin/src/app/api/inbox/actions/dispense/** exclusively) keeps this folder independently
 * editable without reaching into already-merged sibling-action territory.
 */
