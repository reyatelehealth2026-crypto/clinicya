...
/**
 * session.ts — auth/tenant-db resolution for the /api/inbox/actions/recommendations
 * Route Handler (port of api/inbox-v2.php's
 * `case 'recommendations': case 'get_recommendations': case 'drug_recommendations':`,
 * lines ~1191-1350, backed by a 3-tier cascade across
 * `ConsultationAnalyzerService` and `DrugRecommendEngineService::getForSymptoms()`
 * (see `_lib/recommendations.ts`'s module doc).
 *
 ...
 * api/inbox/actions/{check-interactions,check-drug-interactions,
 * suggest-alternatives,safe-alternatives,validate-recommendation,
 * recommendations,drug-card}/** independently editable from sibling action
 * families without cross-imports (the deliberate exception being the
 * read-only cross-batch imports documented in this batch's runbook — e.g.
 * `checkDrugInteractions` from `../../patient-profile/_lib/patientProfile.ts`,
 * mirroring the precedent set by Phase 4 batch 4a's drug-info -> max-discount
 * import).
 */
