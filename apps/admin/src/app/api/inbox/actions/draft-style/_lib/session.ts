...
/**
 * session.ts — auth/tenant-db resolution for the /api/inbox/actions/draft-style
 * Route Handler (port of api/inbox-v2.php's `case 'draft_style': case
 * 'draft-style':`, lines ~397-421).
 *
 * Same "who is logged in + which tenant DB" gate as
 * api/inbox/actions/get-admins/_lib/session.ts and every sibling
 * actions/{tags,notes,medical,send-message,assign-conversation,drug-info}/_lib/session.ts
 ...
 * This batch's ownership boundary is
 * api/inbox/actions/{ghost-draft,learn-draft,draft-style,customer-health,
 * classify-customer}/** — draft-style's own db handle is unused by
 * `_lib/draftStyle.ts` (that function is pure, DB-free) but is still
 * resolved here so the auth gate is uniform across every action in this
 * family, matching house style.
 */
