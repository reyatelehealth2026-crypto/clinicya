...
/**
 * requireTenantPageContext — local copy for `(tenant)/inbox/layout.tsx` and
 * `(tenant)/inbox/page.tsx`. Same duplication rationale as
 * `(tenant)/users/_lib/session.ts`'s own doc comment (and this codebase's
 * other per-feature copies, e.g. `(tenant)/inbox/[userId]/_lib/session.ts`):
 * `(tenant)/layout.tsx` resolves+gates the session for the page shell but
 * doesn't hand it down as a prop, so every leaf layout/page that needs the
 * session/db directly re-resolves it here. inbox-v2.php has no page-specific
 * role gate beyond "any authenticated admin" (`require_once
 * 'includes/header.php'` at line 991, same plain include as
 * users.php/user-detail.php), so the allow-list is every TenantRole.
 *
