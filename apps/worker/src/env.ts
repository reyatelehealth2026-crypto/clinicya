 ...
 * WORKER_HEALTH_PORT, WORKER_HEARTBEAT_INTERVAL_MS,
 * WORKER_SHUTDOWN_TIMEOUT_MS, and WORKER_REALTIME_PORT are read directly off
 * process.env here, NOT added to @reya/config's zod envSchema, because
 * packages/config/** is a READ-ONLY input to this batch (see this repo's
 * mig-worker agent brief's "allowed paths: apps/worker/** only"). Long-term
 * these four belong in the shared schema alongside REDIS_URL so every
 * consumer gets the same validated/typed shape — this file is a
 * deliberately temporary work-around, not the intended final home for these
 * vars.
 */

export interface WorkerEnv extends SharedEnv {
  /** Port the plain node:http health server (health/server.ts) listens on. Default 8099 — chosen to avoid every port already claimed by docker-compose.dev.yml / infra/compose/docker-compose.strangler.yml / infra/e2e/docker-compose.yml (3000, 3001, 3306, 3307, 4000, 6379, 8080, 8091, 16379, 18092, 18199), plus this file's own WORKER_REALTIME_PORT (8100). */
  ...
  /** Port the dedicated Socket.io realtime server (realtime/socketServer.ts) listens on — separate http.Server from WORKER_HEALTH_PORT's plain node:http health server. Default 8100 — chosen to avoid every port already claimed (see WORKER_HEALTH_PORT's doc comment above, including that var's own 8099). */
  WORKER_REALTIME_PORT: number;
}
...
const DEFAULT_WORKER_REALTIME_PORT = 8100;
...
export function loadWorkerEnv(source: NodeJS.ProcessEnv = process.env): WorkerEnv {
  ...
  return {
    ...
    WORKER_REALTIME_PORT: parsePositiveInt(
      source.WORKER_REALTIME_PORT,
      DEFAULT_WORKER_REALTIME_PORT,
      'WORKER_REALTIME_PORT'
    ),
  };
}
