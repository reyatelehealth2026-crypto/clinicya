...
- **Decisions that constrain this work:** `docs/adr/0001-database-per-tenant-isolation.md` (§"Connection routing" = the resolution order and pool contract you are porting; §"Hosting constraint" = why the DB name is hardcoded), `docs/adr/0006-two-realm-session-model.md` (§"Session model" = the two realms, why sessions not JWT, the never-re-hash-bcrypt rule), `docs/adr/0002-tenant-provisioning-pipeline.md`. Do not re-decide these; if a port must diverge, write a superseding ADR.
